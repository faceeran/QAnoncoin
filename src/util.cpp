...
const char * const BITCOIN_CONF_FILENAME = "qanoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "qanoncoin.pid";
...
#else
    const char* pszModule = "qanoncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QAnoncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QAnoncoin";
#else
    // Unix
    return pathRet / ".qanoncoin";
#endif
#endif
}
